...
| **25** | **`N_29DB`** | **-29 dB** | **~790 mV** | **一般（当前）** |
| 40 | `N_14DB` | -14 dB | 约 1.6 V | 比较响 |
| 50 | `N_5DB` | -5 dB | 约 2.2 V | 很大 |
| 53 | `N_0DB` | 0 dB | 接近上限 | 非常大 |
| 59 | `P_5DB` | +5 dB | **可能削顶失真** | 超大但开始失真 |
...
1. **频率与音量完全独立**：avol 从 25 改到 50（+24 dB 提升），频率仍稳定在 1.9598 kHz，标准偏差 0.0 Hz。**频率不会被音量影响**。
2. **avol 索引 = 50 对应 `N_5DB`**：
   - `tbl_dac_avol_gain[50] = N_5DB`（-5 dB 模拟增益）；
   - 寄存器值 = `(AUANGCON3 & ~0x7f) | 0x6B`；
   - 索引越大 dB 越高（接近 0 dB），索引 53 是 `N_0DB`（0 dB 临界）；
   - 索引 54~59 是 `P_1DB` ~ `P_5DB`（正增益，会让 Vpp 更大，可能削顶）。
3. **数字音量保持最大**：dvol 仍是 `DIG_N0DB`（0x7FFF），说明 `dac_set_dvol` 没被这步改动影响。
4. **DAC 内部模拟通路工作正常**：AUANGCON3 写入 0x6B 后 DAC 模拟输出明显增大，没出现静音、噪声或失真。
